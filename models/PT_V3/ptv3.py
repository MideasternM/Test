...
def deepcopy_point(point):
    if isinstance(point, dict):
        new_point = Point()
        for key, value in point.items():
            new_point[key] = deepcopy_point(value)
        return new_point
    elif isinstance(point, list):
        return [deepcopy_point(item) for item in point]
    elif isinstance(point, spconv.SparseConvTensor):
        return spconv.SparseConvTensor(
            features=point.features.clone(),
            indices=point.indices.clone(),
            spatial_shape=copy.deepcopy(point.spatial_shape),
            batch_size=point.batch_size
        )
    elif isinstance(point, torch.Tensor):
        return point.clone()
    elif isinstance(point, (int, float, str, bool, type(None))):
        return point
    else:
        raise TypeError(f"Unsupported type in Point: {type(point)}")
    ...
    def __init__(
        ...
        for i in num_classes:
            seg_head = nn.Sequential(
                ...
                nn.LayerNorm(dec_channels[0]),
                nn.GELU(),
                nn.Linear(dec_channels[0], i)
            )
